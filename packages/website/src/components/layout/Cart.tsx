...
const Cart: FC = () => {
    ...
    return (
        <Col>
            ...
            >
                {store.cart.length ? product.map((item?: Product, index?) => {
                    return (
                        <Row key={item?._id} gutter={[40, 0]} justify={"center"}>
                            ...
                            >
                                ...
                                <Typography.Paragraph>
                                    <ButtonCart description={'-'} option={'delete'} type={'ghost'}
                                                productId={store.cart[index]?.product}/>
                                    &nbsp;{store.cart[index]?.quantity.toString()}&nbsp;
                                    <ButtonCart description={'+'} option={'add'} type={'ghost'}
                                                productId={store.cart[index]?.product}/>
                                </Typography.Paragraph>
                            </Col>
                            <Col span={6}>
                ...
                }) : <Typography.Title style={{textAlign: 'center'}} level={3}>Cart is empty</Typography.Title>
                }
            </Modal>
        </Col>
    );
