...
// const EmployeeSlide: FC<any> = ({ photo, occupation, name }) => (
//     <div className='staff-list-item-wrapper'>
//         <div className='staff-list-item-picture'>{photo && <img src={photo.url} className='staff-list-item-img' />}</div>
//         <p className='staff-list-item-name'>{name}</p>
//         <p className='staff-list-item-position'>{occupation}</p>
//     </div>
// )
                    ...
                    <li className='item' key={item._id}>
                        <div className='item-wrapper'>
                            <div style={{width: `${width}vw`}} className='img-block'>
                                <Image src={item.photo ? item.photo : placeholders[index % 3]} className='img'/>
                            </div>
...
const StaffGrid: FC<Partial<{ data: Person[], defaultItemsToShow: number }>> = ({
                                                                                    data = [],
                                                                                    defaultItemsToShow = 13
                                                                                }) => {
    ...
    return (
        <ul className={`about-list ${viewMoreClass}`}>
            {data?.map((item, index) => {
                        ...
                        <Fragment key={index}>
                            <li className={'more-wrapper'}>
                                <div className='more-block' onClick={handleOpenStaff}>
                                    <p style={{display: isViewMore ? 'none' : 'initial'}}
                            ...
                            <li className='item'>
                                ...
                                <div className='item-wrapper'>
                                    ...
                                    {item.occupation && <p className='occupation'>{item.occupation}</p>}
                                </div>
                            </li>
                        </Fragment>
                    )
                ...
                return (
                    <li className='item' key={index} style={index >= 10 ? {animationDelay: `0.${index}s`} : {}}>
                        <div className='item-wrapper'>
                            <div className='img-block'>
                                <img src={item.photo ? item.photo : placeholders[index % 3]} className='img'/>
                            </div>
                            {item.name && <p className='name'>{item.name}</p>}
                            {item.occupation && <p className='occupation'>{item.occupation}</p>}
                        </div>
...
const We: FC = () => {
    ...
    const filteredAndSorted = we.filter(obj => pinnedOccupations.includes(obj.occupation))
        .sort((a, b) => pinnedOccupations.indexOf(a.occupation) - pinnedOccupations.indexOf(b.occupation));

    const mixed = we.filter(obj => !pinnedOccupations.includes(obj.occupation))
